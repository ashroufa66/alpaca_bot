"""
╔══════════════════════════════════════════════════════════════╗
║         Quantitative Trading Bot  —  Version V10.9          ║
...
TRADE_LOG_FILE  = "trade_log_v10_9.csv"
...
def try_enter(symbol: str) -> bool:
    ...
    if order:
        ...
        write_trade_log("BUY_SUBMITTED", symbol, qty, ask,
                        "V10_9_MOMENTUM", ai_prob, kelly_pct, "momentum")
        return True
...
async def main():
    log("=" * 65)
    log("Quantitative Trading Bot V10.9 — Starting up")
    log("─" * 65)
    log("Fixes in V10.9:")
    log("   • Large-cap whitelist — NVDA/AMD/AAPL always in candidates")
    log("   • HALT timeout raised 180s -> 300s for IEX sparse bars")
    log("   • Relative volume relaxed 0.8 -> 0.5 for IEX")
    log("   • Whitelist injected after every scan cycle")
    log("   • All V10.8 fixes carried over")
    log("─" * 65)
