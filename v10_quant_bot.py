...
# For most paper accounts, IEX is safer than SIP.
DATA_FEED = "iex"
...
MIN_OPENING_GAP_PCT = 1.5
MAX_OPENING_GAP_PCT = 20.0
...
TRADE_LOG_FILE = "trade_log_v10.csv"
...
state = {
    ...
    "positions": {},               # symbol -> {entry_price, qty, highest_price, atr_at_entry, stop_price, tp_price, sector}
...
def get_sector(symbol: str) -> str:
    cached = state["sector_cache"].get(symbol)
    if cached:
        return cached

    # Alpaca assets endpoint does not reliably provide GICS sector for equities.
    # Best effort: infer broad bucket from symbol characteristics; default "unknown".
    # This keeps the filter functional but conservative.
    sector = "unknown"
    state["sector_cache"][symbol] = sector
    return sector
...
def detect_market_regime(force: bool = False) -> str:
    now = time.time()
    if not force and (now - state["last_regime_check"] < 120):
...
def try_enter(symbol: str):
    ...
    if order:
        ...
        write_trade_log("BUY_SUBMITTED", symbol, qty, ask, "V10_QUANT_ENTRY")
            ...
            async with websockets.connect(TRADE_STREAM_URL, ping_interval=20, ping_timeout=20) as ws:
                ...
                while True:
                    ...
                    if event == "partial_fill":
                        ...
                        log(f"PARTIAL FILL {symbol} side={side} qty={filled_qty} avg={filled_avg_price:.2f}")
                    ...
                    if event in ("fill", "partial_fill", "canceled", "rejected", "expired"):
                        if event in ("fill", "canceled", "rejected", "expired"):
                            state["pending_symbols"].discard(symbol)
                            state["pending_orders"].pop(order_id, None)
...
async def main():
    log("V10 Quant Bot started")
    log("Mode: 5000 scan -> AI momentum rank v2 -> top 10 -> regime filter -> websocket execution -> EMA + VWAP + ATR + news + halt protection")
