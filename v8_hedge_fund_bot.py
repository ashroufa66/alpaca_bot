...
# Use SIP if you have it. Otherwise switch to IEX.
...
MIN_OPENING_GAP_PCT = 2.0
MAX_OPENING_GAP_PCT = 18.0
MIN_DAY_CHANGE_PCT = 1.5
MIN_MINUTE_MOMENTUM_PCT = 0.08
...
NEGATIVE_NEWS_KEYWORDS = [
    ...
    "missed earnings", "secondary offering", "restatement"
]
...
TRADE_LOG_FILE = "trade_log_v8.csv"
...
state = {
    ...
    "positions": {},               # symbol -> {entry_price, qty, highest_price, atr_at_entry, stop_price, tp_price}
    "pending_orders": {},          # order_id -> {symbol, side, submitted_at, qty}
    "pending_symbols": set(),
    "cooldowns": {},
    "trades_today": 0,
...
def reset_daily_if_needed():
    ...
    if state["current_day"] != today:
        ...
        state["news_cache"] = {}
        state["market_open_today"] = None
        state["market_close_today"] = None
...
def refresh_clock_times():
    try:
        c = get_clock()
        state["market_open_today"] = pd.to_datetime(c.get("next_open") if not c.get("is_open") else c.get("timestamp"))
        state["market_close_today"] = pd.to_datetime(c.get("next_close"))
    except Exception as e:
        log(f"refresh clock times error: {e}")
...
def get_open_orders() -> list:
    r = requests.get(f"{TRADE_BASE_URL}/v2/orders", headers=HEADERS, params={"status": "open"}, timeout=20)
    r.raise_for_status()
    return r.json()
...
def close_position_market(symbol: str):
    ...
    r = requests.delete(f"{TRADE_BASE_URL}/v2/positions/{symbol}", headers=HEADERS, timeout=20)
    return r.status_code in (200, 204)
...
def minutes_since_market_open() -> float:
    try:
        ...
        next_open = pd.to_datetime(clock["next_open"])
        next_close = pd.to_datetime(clock["next_close"])

        # if market open, today's open is close - session length inference isn't reliable.
        # simpler: use 9:30 ET on current date when open
...
def detect_halt(symbol: str) -> bool:
    ...
    if bid <= 0 or ask <= 0:
        log(f"HALT suspected {symbol}: missing bid/ask")
        return True
...
def calc_momentum_score(symbol: str, snap: dict):
    ...
    score = (
        day_change_pct * 1.8
        + minute_momentum_pct * 4.5
        + min(relative_volume, 10) * 2.2
        + min(dollar_volume / 10_000_000, 10) * 1.0
        + news_score
        - sp * 4.0
    )
...
def run_scanner():
    ...
    if top:
        ...
        log(f"Scanner 3000 -> top {TOP_CANDIDATES}: {preview}")
    else:
...
def calc_dynamic_qty(entry_price: float, atr_value: float) -> int:
    ...
    account_risk_dollars = max(state["account_equity"], buying_power) * ACCOUNT_RISK_PCT
...
def try_enter(symbol: str):
    ...
    if order:
        ...
        log(f"BUY SUBMITTED {symbol} qty={qty} ask={ask:.2f} atr={atr_value:.4f}")
        write_trade_log("BUY_SUBMITTED", symbol, qty, ask, "EMA_VWAP_ATR_ENTRY")
...
def cleanup_stale_pending_orders():
    now_ts = time.time()
    for order_id, info in list(state["pending_orders"].items()):
        if now_ts - info.get("submitted_at", now_ts) > ORDER_TIMEOUT_SECONDS:
        ...
        try:
            ...
            if market_is_open():
                ...
                log(
                    ...
                    f"Top={','.join(state['scanner_candidates'][:8]) if state['scanner_candidates'] else 'none'} | "
...
async def main():
    log("V8 Hedge Fund Bot started")
    log("Mode: 3000 scan -> top candidates -> websocket execution -> EMA + VWAP + ATR + news + halt protection")
