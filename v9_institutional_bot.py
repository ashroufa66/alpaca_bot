...
# For paper accounts, IEX is usually safer.
DATA_FEED = "iex"
...
TRADE_LOG_FILE = "trade_log_v9.csv"
...
def calc_ai_momentum_score(symbol: str, snap: dict):
    ...
    # Heuristic "AI" score: weighted factors
    score = (
        day_change_pct * 1.6
        + minute_momentum_pct * 4.8
        + min(relative_volume, 10) * 2.4
        + min(dollar_volume / 10_000_000, 10) * 1.2
        + min(minute_range_pct, 8.0) * 1.4
        + news_score * 1.0
        - sp * 4.5
    )
...
def try_enter(symbol: str):
    ...
    if order:
        ...
        write_trade_log("BUY_SUBMITTED", symbol, qty, ask, "V9_AI_MOMENTUM_ENTRY")
...
async def main():
    log("V9 Institutional Bot started")
    log("Mode: 5000 scan -> AI momentum score -> top 30 -> websocket execution -> EMA + VWAP + ATR + news + halt protection")
