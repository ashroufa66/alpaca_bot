"""
╔══════════════════════════════════════════════════════════════╗
║         Quantitative Trading Bot  —  Version V10.10          ║
...
MIN_RELATIVE_VOLUME     = 0.5   # FIX V10.9: IEX volume understated — relaxed from 0.8
...
MAX_POSITION_USD          = 500.0   # V10.10: raised 300->500 — bigger positions = more profit
...
TRADE_LOG_FILE  = "trade_log_v10_10.csv"
...
def submit_market_order(symbol: str, qty: int, side: str) -> Optional[dict]:
    """
    V10.10: Market order for BUY entries — guarantees fill.
    Limit orders on IEX often miss fills due to stale quotes.
    Sell orders remain limit to avoid selling into bad spreads.
    """
    payload = {
        "symbol": symbol, "qty": str(qty), "side": side,
...
def try_enter(symbol: str) -> bool:
    ...
    # V10.10: use market order for buys — IEX limit orders often miss fills
...
async def main():
    log("=" * 65)
    log("Quantitative Trading Bot V10.10 — Starting up")
    log("─" * 65)
    log("Fixes in V10.10:")
    log("   • Take Profit raised 2.2x -> 3.0x — let winners run")
    log("   • Stop multiplier raised 1.0 -> 1.5 — more room to breathe")
    log("   • Position size raised $300 -> $500 — bigger profits")
    log("   • BUY orders now MARKET type — guaranteed fills on IEX")
    log("   • Order timeout reduced 25s -> 20s — faster cleanup")
    log("   • All V10.9 fixes carried over")
    log("─" * 65)
