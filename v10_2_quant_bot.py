...
TRADE_STREAM_URL = (
    "wss://paper-api.alpaca.markets/stream" if PAPER else "wss://api.alpaca.markets/stream"
)
...
CHOP_MIN_SCORE = 8.0  # FIX #5: deterministic threshold instead of random bypass
...
ORDER_TIMEOUT_SECONDS = 25  # FIX #7: unified, removed duplicate constant

# Timing
COOLDOWN_SECONDS = 20 * 60
REENTRY_BLOCK_MINUTES = 45
HALT_TIMEOUT_SECONDS = 180  # FIX #10: reduced from 600 to 180 seconds
...
NEWS_CACHE_TTL_SECONDS = 120  # FIX #11: reduced from 300 to 120 seconds
NEWS_LIMIT = 8
...
TRADE_LOG_FILE = "trade_log_v10_2.csv"
...
state: Dict = {
    ...
    "indicator_cache": {},      # symbol -> {bar_count, df}  FIX #8
    ...
    "market_open_time": None,   # FIX #2: track open time for relative volume
}
...
def get_indicators(symbol: str) -> pd.DataFrame:
    """Return a cached indicator DataFrame, recomputing only when bar count changed."""
...
def sector_position_count(sector: str) -> int:
    if not sector or sector == "unknown":
        return 0
    return sum(1 for pos in state["positions"].values() if pos.get("sector") == sector)
...
def load_scan_universe():
    try:
        ...
        # FIX #12: warn if universe is being capped
...
def minutes_since_market_open() -> float:
    """FIX #2: cache market open time to avoid repeated clock calls."""
...
def analyze_news(symbol: str) -> Tuple[str, int]:
    ...
    if cached and now - cached["ts"] < NEWS_CACHE_TTL_SECONDS:  # FIX #11
...
def detect_halt(symbol: str) -> bool:
    ...
    # FIX #10: 180s threshold instead of 600s
...
def atr_ok(df: pd.DataFrame) -> bool:
    ...
    if atr <= 0 or close <= 0:
        return False
    return (atr / close) * 100.0 >= MIN_ATR_PCT
...
def intraday_strength(df: pd.DataFrame) -> float:
    ...
    if high <= low:
        return 0.5  # FIX #6: neutral fallback instead of blocking entry
...
def opening_momentum_filter(prev_close: float, price: float) -> bool:
    ...
    if gap < MIN_OPENING_GAP_PCT or gap > MAX_OPENING_GAP_PCT:
        return False
...
def calc_relative_volume(daily_volume: float, minute_volume: float) -> float:
    """
    FIX #2: Use elapsed minutes since market open as the denominator so that
    relative volume is accurate early in the session instead of always being
    underestimated when daily_volume is a partial figure.
    """
    if daily_volume <= 0 or minute_volume <= 0:
        return 0.0

    elapsed = max(minutes_since_market_open(), 1.0)
    expected_minute_volume = daily_volume / elapsed
...
def run_scanner():
    ...
    if top:
        ...
        log(f"Scanner {len(state['all_symbols'])} -> top {TOP_CANDIDATES}: {preview}")
    else:
...
def entry_quality_ok(symbol: str, df: pd.DataFrame) -> bool:
    ...
    if strength < 0.35:
        log(f"Entry quality blocked {symbol}: intraday_strength={strength:.2f}")
        return False
...
def try_enter(symbol: str) -> bool:
    ...
    # FIX #5: deterministic score threshold in chop, not random bypass
    if regime == "chop":
    ...
    # FIX #9: consistent spread check using stored value
    if q["spread_pct"] > MAX_SPREAD_PCT:
        return False

    df = get_indicators(symbol)  # FIX #8: use cache
    ...
    # FIX #4: volume spike is now a hard required condition, no random bypass
    ...
    if order:
        ...
        write_trade_log("BUY_SUBMITTED", symbol, qty, ask, "V10_2_QUANT_ENTRY")
        return True

    return False

def try_exit(symbol: str) -> bool:
    # FIX #3: guard both conditions atomically before any mutation
    ...
    stop_price = pos.get("stop_price") or (entry - atr_at_entry * ATR_STOP_MULT)
...
def close_all_positions():
    for symbol in list(state["positions"].keys()):
        # FIX #3: use the same guard as try_exit
...
def cleanup_old_orders():
    ...
    for order_id, data in list(state["pending_orders"].items()):
        # FIX #7: use unified ORDER_TIMEOUT_SECONDS
                ...
                while True:
                    ...
                    for item in data:
                        ...
                        if typ == "q":
                            ...
                            # Invalidate indicator cache on new bar
                            state["indicator_cache"].pop(symbol, None)  # FIX #8
            ...
            async with websockets.connect(TRADE_STREAM_URL, ping_interval=20, ping_timeout=20) as ws:
                ...
                while True:
                    ...
                    if event == "partial_fill":
                        cumulative_qty = int(float(order.get("filled_qty", 0) or 0))
                        filled_avg_price = float(order.get("filled_avg_price", 0) or 0)
                        previously_seen = int(order_meta.get("filled_qty_seen", 0))
                        incremental_qty = max(cumulative_qty - previously_seen, 0)

                        if incremental_qty > 0:
                            order_meta["filled_qty_seen"] = cumulative_qty
                            ...
                            if side == "buy":
                                if symbol not in state["positions"]:
                                    ...
                                    if new_qty > 0:
                                        pos["entry_price"] = (
                                            (old_entry * old_qty) + (filled_avg_price * incremental_qty)
                                        ) / new_qty
                                    pos["qty"] = new_qty
                        ...
                        log(
                            f"PARTIAL FILL {symbol} side={side} "
                            f"cumulative_qty={cumulative_qty} avg={filled_avg_price:.2f}"
                        )
                    ...
                    # ------ full fill ------
                        ...
                        if side == "buy":
                            df = get_indicators(symbol)  # FIX #8
                            atr_at_entry = 0.0
                            if not df.empty:
                                atr_at_entry = float(df["atr"].iloc[-1] or 0)

                            stop_price = (
                                filled_avg_price - atr_at_entry * ATR_STOP_MULT
                                if atr_at_entry > 0
                                else filled_avg_price * 0.99
                            )
                            ...
                            # FIX #1: only credit the qty not already credited via partial fills
                            previously_seen = int(order_meta.get("filled_qty_seen", 0))
                            incremental_qty = max(filled_qty - previously_seen, 0)

                            if incremental_qty > 0 and symbol in state["positions"]:
                                entry = float(state["positions"][symbol].get("entry_price", filled_avg_price))
                                pnl = (filled_avg_price - entry) * incremental_qty
                                state["realized_pnl_today"] += pnl
                                log(
                                    f"SELL FILLED {symbol} qty={filled_qty} "
                                    f"price={filled_avg_price:.2f} pnl={pnl:.2f} "
                                    f"(incremental={incremental_qty})"
                                )
                            else:
                                log(
                                    f"SELL FILLED {symbol} qty={filled_qty} "
                                    f"price={filled_avg_price:.2f} (already credited via partials)"
                                )
    ...
    while True:
        try:
            ...
            if market_is_open():
                ...
                top_names = (
                    ",".join(state["scanner_candidates"][:8])
                    if state["scanner_candidates"] else "none"
                )
                ws_names = (
                    ",".join(state["ws_symbols"][:8])
                    if state["ws_symbols"] else "none"
                )
...
async def main():
    log("V10.2 Quant Bot started")
    log(
        "Fixes: PnL double-count, relative volume denominator, race condition, "
        "volume spike hard filter, chop regime deterministic threshold, "
        "indicator cache, unified timeout, halt sensitivity, news cache TTL, "
        "universe cap warning, spread consistency, intraday_strength edge case"
    )
