...
TRADE_LOG_FILE  = "trade_log_v10_8.csv"
...
def try_enter(symbol: str) -> bool:
    ...
    if order:
        ...
        write_trade_log("BUY_SUBMITTED", symbol, qty, ask,
                        "V10_8_MOMENTUM", ai_prob, kelly_pct, "momentum")
        return True
...
async def main():
    log("=" * 65)
    log("Quantitative Trading Bot V10.8 — Starting up")
    log("─" * 65)
    log("Fixes in V10.8:")
    log("   • Spread prediction DISABLED for IEX — not reliable")
    log("   • Dynamic spread limit: 5x relaxed on IEX feed")
    log("   • WebSocket ping interval raised to 30s — less drops")
    log("   • Auth format: new Alpaca standard")
    log("   • HALT log: max once per 60s per symbol")
    log("   • Reconnect: only on 5+ symbol changes")
    log("─" * 65)
