...
POSITIVE_NEWS_KEYWORDS = [
    ...
    "contract", "approval", "fda approval", "acquisition"
]
...
def run_scanner():
    ...
    if top:
        ...
        log(f"Scanner 3000->20 top: {preview}")
    else:
...
def try_enter(symbol: str):
    ...
    if order:
        ...
        write_trade_log("BUY_SUBMITTED", symbol, qty, ask, "ELITE_SCANNER_ENTRY")
    ...
    while True:
        try:
            async with websockets.connect(DATA_WS_URL, ping_interval=20, ping_timeout=20) as ws:
                ...
                # start with top 20 if available, otherwise first 20 from universe
                ...
                while True:
                    # if scanner changed top 20, reconnect with new symbols
...
async def main():
    log("V4 Elite Scanner Bot started")
    log("Mode: 3000 scan -> top 20 websocket -> trade best setup")
