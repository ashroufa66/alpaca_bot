"""
╔══════════════════════════════════════════════════════════════╗
║         Quantitative Trading Bot  —  Version V12.4          ║
...
SIMULATED_ACCOUNT_SIZE = 500.0

# ── Scanner ──
MAX_SCAN_SYMBOLS      = 2000   # FIX V12.2: 5000->2000 — actual scan is 1500, keep buffer
...
# ── Risk Management ──
MAX_OPEN_POSITIONS        = 2     # FIX V11.4: $500 account cannot handle 5 positions
MAX_TRADES_PER_DAY        = 10    # FIX V12.2: 20->10 — $500 account, quality over quantity
DAILY_MAX_LOSS_USD        = 25.0   # FIX V11.4: 50->25 — 5% max daily loss on $500 account
MAX_POSITION_USD          = 200.0   # FIX V11.3: 500->200 — max ~40% account per trade (SAFER)
MIN_POSITION_USD          = 50.0   # FIX V11.5: 30->50 — small trades have worse slippage
...
VWAP_MODEL_MIN_SAMPLES   = 100  # FIX V12.1: 20->100 — ML needs 100+ samples minimum
VWAP_REVERSION_MIN_PROB  = 0.60 # minimum win probability to enter a reversion trade
VWAP_DEVIATION_MIN_PCT   = 0.6   # base value — overridden dynamically for IEX (see below)
VWAP_FEATURE_NAMES = [
...
TRADE_LOG_FILE  = "trade_log_v12_4.csv"
...
async def async_get_account() -> dict:
    """FIX V12.2: non-blocking account fetch."""
...
async def run_dynamic_scanner():
    ...
    try:
        ...
        sample   = state["all_symbols"][:1500]   # FIX V12.1: 3000->1500 — balance coverage vs speed
        results  = []
        ...
        for batch in chunks(sample, SNAPSHOT_BATCH_SIZE):
            try:
                ...
                pass
            await asyncio.sleep(0.1)   # FIX V12.4: non-blocking
...
async def run_scanner():
    ...
    for batch in chunks(priority_list[:MAX_SCAN_SYMBOLS], SNAPSHOT_BATCH_SIZE):
        ...
        await asyncio.sleep(0.18)  # FIX V12.4: non-blocking
...
def calc_kelly_qty(symbol: str, entry_price: float, atr_value: float,
    ...
    qty = int(max(min(qty_risk, qty_cap), 0) * risk_scale() * vol_factor * spread_factor * of_factor * vix_factor)
    return max(qty, 0)
...
def get_order_flow_factor(symbol: str) -> float:
    ...
    """
    if not ORDER_FLOW_ENABLED or DATA_FEED == "iex":
        return 1.0   # FIX V12.1: IEX velocity not trustworthy
...
def try_partial_exit(symbol: str) -> bool:
    """
    ...
    order    = submit_limit_order(symbol, sell_qty, "sell", bid)
    if order:
        oid = order["id"]
        state["pending_orders"][oid] = {
...
async def try_enter(symbol: str) -> bool:
    ...
    tod_quality = time_of_day_quality()
    ...
    log(f"🟢 BUY {symbol} qty={qty} smart_exec "
        ...
        f"of={of_factor:.2f} vix={state['vix_proxy_regime']} regime={regime}")
                    ...
                    for item in data:
                        ...
                        if typ == "q":
                            ...
                            if symbol == "SPY":
                                ...
                                try_partial_exit(symbol)   # V11.0: partial profit first
                                try_exit(symbol)
        ...
        try:
            ...
            if market_is_open():
                ...
                log(
                    ...
                    f"VIX={state['vix_proxy_regime'].upper()}({state['vix_proxy_value']:.2f}x) | "
...
async def main():
    log("=" * 65)
    log("Quantitative Trading Bot V12.4 — Starting up")
    log("─" * 65)
    log("Async polish in V12.4:")
    log("   • time.sleep → asyncio.sleep everywhere (event loop unblocked)")
    log("   • SNAPSHOT_BATCH_SIZE: 200->150 (lower latency per batch)")
    log("   • Trade log: async queue worker (CSV writes non-blocking)")
    log("   • All V12.3 async fixes carried over")
    log("─" * 65)
